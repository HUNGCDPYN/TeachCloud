...
public class PowerDatacenter extends Datacenter {
	...
	protected double updateCloudetProcessingWithoutSchedulingFutureEventsForce() {
		...
		Log.formatLine("New resource usage for the time frame starting at %.2f:", currentTime);
                HostStatisticalGenerator.getInstance().addTime(currentTime);
		for (PowerHost host : this.<PowerHost> getHostList()) {
			...
			Log.formatLine(
					...
					host.getUtilizationOfCpu() * 100);
                       ...
                       HostStatisticalGenerator.getInstance().addUtilization(host.getId(), currentTime, host.getUtilizationOfCpu());
		}
		...
		if (timeDiff > 0) {
			...
			for (PowerHost host : this.<PowerHost> getHostList()) {
				...
				Log.formatLine(
						...
						host.getId(),
						timeFrameHostEnergy);
                                HostStatisticalGenerator.getInstance().addEnergy(host.getId(), currentTime, timeFrameHostEnergy);
			}
